...
# Cette API est 100% gratuite et sans clé API
...
ENTREPRISES = [
    ...
    "EATON INTERCONNECT TECHNOLOGIES DIVISION", "EATON SAS",
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if resultats:
                ...
                return {
                    "siren": e.get("siren", ""),
                    "denomination_officielle": e.get("nom_complet", "") or e.get("nom_raison_sociale", ""),
                    "siege_ville": e.get("siege", {}).get("commune", "") if e.get("siege") else "",
                    "nb_resultats": len(resultats),
                    "statut": "Trouve"
                }
            else:
                return {"siren": "", "denomination_officielle": "", "siege_ville": "", "nb_resultats": 0, "statut": "Non trouve"}
        else:
            return {"siren": "", "denomination_officielle": "", "siege_ville": "", "nb_resultats": 0, "statut": f"Erreur {r.status_code}"}
    except Exception as e:
        return {"siren": "", "denomination_officielle": "", "siege_ville": "", "nb_resultats": 0, "statut": f"Erreur: {str(e)[:50]}"}
...
def creer_excel(resultats):
    ...
    headers = ["Nom recherche", "SIREN", "Denomination officielle", "Ville siege", "Statut", "Nb resultats"]
    largeurs = [45, 15, 45, 20, 15, 15]
    ...
    for i, (nom, res) in enumerate(resultats.items(), 2):
        ...
        row = [nom, res["siren"], res["denomination_officielle"], res["siege_ville"], res["statut"], res["nb_resultats"]]
    ...
    wb.save("sirens_entreprises.xlsx")
    print("Fichier genere : sirens_entreprises.xlsx")
...
if __name__ == "__main__":
    ...
    for i, nom in enumerate(ENTREPRISES, 1):
        ...
        if res["statut"] == "Trouve":
            trouves += 1
            print(f"  OK {res['siren']} - {res['denomination_officielle']} ({res['siege_ville']})")
        else:
